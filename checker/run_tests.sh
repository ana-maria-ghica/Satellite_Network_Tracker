...
# Utilizare:
#   ./run_tests.sh            -> rulează toate task-urile 1-4
...
declare -A TASK_POINTS=( [1]=30 [2]=20 [3]=20 [4]=30 )

# Verificăm că executabilul există
if [[ ! -x "main" ]]; then
  echo "Executabilul 'main' nu a fost găsit sau nu e executabil."
  exit 1
fi
...
for task in "${TASKS[@]}"; do
  ...
  TEST_DIR="checker/$TASKS_DIR/task${task}/tests"
  if [[ ! -d "$TEST_DIR" ]]; then
    ...
    if [[ -d "$test_dir" ]]; then
      ...
      if diff -u -w -B "$ref_file" "$out_file" > /dev/null; then
        ...
        valgrind --leak-check=full --error-exitcode=1 --log-file="$vg_log" \
          --quiet ./main -c${task} "$in_file" "/dev/null"
        vg_rc=$?
        if [[ $vg_rc -eq 0 ]]; then
...
# Rezumat global
echo "======================================"
echo "Scor total: $(printf "%.2f" "$global_score") puncte din 100."
